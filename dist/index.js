...
const clusterEnabled = /true/i.test(process.env.CLUSTER_ENABLED); // enable authorization
...
function clearRoutes() {
  app._router.stack = app._router.stack.filter(k => !(k && k.route && k.route.path && k.route.path.startsWith(apiRoot)));
}
/**
...
async function startMicroLib({
  ...
  if (hot) {
    // clear stale routes
    clearRoutes(); // clear cache on hot reload
...
exports.handleServerlessRequest = async function (...args) {
  ...
  if (!serverlessAdapter) {
    serverlessAdapter = await ServerlessAdapter(startMicroLib, cloudProvider, messageParser);
  }
