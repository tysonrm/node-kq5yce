...
function stopWorker() {
  const worker = reloadList.pop();
  if (worker) worker.kill("SIGTERM");else {
    reloading = false;
...
function continueReload(callback, waitms) {
  const failedWorker = !reloading && workerList.length < numCores && callback.name.includes("start");
...
exports.startCluster = function (startService, waitms = 2000) {
  if (cluster.isMaster) {
    ...
    console.log(`master starting ${numCores} workers 🌎`); // Run a copy of this program on each core
